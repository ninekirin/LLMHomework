...
const MyRequests = () => {
  ...
  useEffect(() => {
    updateUrlAndGetMyRequests();
  }, [filterType, filterStatus, filterOrder]);

  const updateUrlAndGetMyRequests = () => {
    const newParams = {
      ...params,
      current: 1,
    ...
    window.history.pushState({}, '', `/request/list?${new URLSearchParams(newParams).toString()}`);

    getMyRequests({ current: 1, pageSize: tableParams.pagination.pageSize });
  };

  const getMyRequests = params => {
    setLoading(true);
    ...
    fetch(`${apiBaseUrl}/requests/myrequests?${query.toString()}`, {
      method: 'GET',
      headers: {
  ...
  const handleStatusChange = (requestId, currentStatus, newStatus) => {
    ...
    if (newStatus === 'APPROVED' || newStatus === 'REJECTED') {
      message.error('Only REVOKED status can be set.');
      return;
    }
    fetch(`${apiBaseUrl}/request/myrequest`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': token,
    ...
    },
    {
      ...
      render: (_, record) => {
        if (record.request_type === 'UPDATE_SCORE') {
          ...
          // return <Link to={`/experiment/view?id=${record.experiment_id}`}>View Experiment</Link>;
          return (
            <p>
  ...
  return (
    <Card title="List of My Requests" headStyle={{ fontSize: '20px' }}>
      ...
      <Space direction="vertical" size="middle" style={{ display: 'flex', width: '100%' }}>
        ...
        <Table
          ...
          bordered
          onChange={pagination => {
            getMyRequests({ current: pagination.current, pageSize: pagination.pageSize });
            const newParams = {
              ...params,
            ...
            window.history.pushState(
              {},
              '',
              `/request/list?${new URLSearchParams(newParams).toString()}`
            );
          }}
        />
      </Space>
